...
int main()
{
    ...
    // Sort result by profit in decreasing order for final output
    sort(result.begin(), result.end(), [](const vector<float> &a, const vector<float> &b) {
        return a[1] > b[1]; // Sort by profit (index 1) in descending order
    });
